defmodule MapsAsFunctions do
  ...
  defmacro defmap(map_name, map_pairs) do
    ...
    attribute_part = quote location: :keep do
    ...
    mapper_clauses = Enum.map map_pairs, fn({k, v}) ->
    ...
    mapper_fallback_clause = quote do
      def unquote(map_name)(_), do: nil
    end

    strict_mapper_name = :"#{map_name}!"
    strict_mapper_clauses = Enum.map map_pairs, fn({k, v}) ->
      escaped_k = Macro.escape(k)
      escaped_v = Macro.escape(v)
      quote location: :keep do
    ...
    predicate_clauses = Enum.map map_pairs, fn({k, _}) ->
      escaped_k = Macro.escape(k)
      quote location: :keep do
